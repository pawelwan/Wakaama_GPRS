...
int sdGetSizeInfo(uint32_t *NumberOfBlocks, uint16_t *BlockSize)
{
	...
	*BlockSize =  512;
	*NumberOfBlocks =  SDCardInfo.CardCapacity / 512; // CardCapacity is uint64_t
	
	xprintf("sdGetSizeInfo: nb of blocks = %d\n",(int)*NumberOfBlocks);
...
{
	if( SD_ReadMultiBlocksFIXED (Buffer,
						  BlockAddress,
						  512,
						  1) != 0)
	{
...
{
	if( SD_WriteMultiBlocksFIXED (Buffer,
						   BlockAddress,
						   512,
						   1) != 0)
	{
