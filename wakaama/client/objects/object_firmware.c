...
#define RES_M_FIRMWARE_UPDATE_DELIVERY_METHOD   9

typedef struct{
    char package_url[100];
    int state;
    bool supported;
    int result;
    int method;
                                 ...
                                 lwm2m_object_t * objectP)
{
    ...
    do
    {
        ...
        case RES_M_FIRMWARE_UPDATE_DELIVERY_METHOD:
            lwm2m_data_encode_int(data->method, *dataArrayP + i);
            result = COAP_205_CONTENT;
            break;
                                    ...
                                    lwm2m_object_t * objectP)
{
    ...
    case RES_M_UPDATE:
        if (data->state == 1)
        {
            ...
            downloadFirmware(data->package_url, *lwm2mContext);
            fprintf(stdout, "\r\n\t FIRMWARE UPDATE\r\n");
            //update
            data->state = 3;
            // update done
...
lwm2m_object_t * get_object_firmware(lwm2m_context_t ** context)
{
    ...
    if (NULL != firmwareObj)
    {
        ...
        if (NULL != firmwareObj->userData)
        {
            ...
            ((firmware_data_t*)firmwareObj->userData)->method = 0;
            lwm2mContext = context;
        }
        else
        {
            lwm2m_free(firmwareObj);
